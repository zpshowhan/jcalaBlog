...
public class FileTools {
	 public static final String CLASSPATH = new File(FileTools.class.getResource("/").getPath()).getPath() + File.separatorChar;
    /**
     * 获取文件后缀
     */
    ...
    public static String updatePic(String restUrl,String picHome,HttpServletRequest request)
        ...
        if(!multipartFile.isEmpty()){
        	...
        	File path = new File(webPath+picHome+File.separatorChar+ yearMonth);
        	
        	//合成图片在服务器上的绝对路径
        	File targetFile = new File(webPath+picHome+File.separatorChar + yearMonth + File.separatorChar + fileName);
        	if (!path.exists()) {
        		path.mkdirs();
        	}
        	//保存图片
        	multipartFile.transferTo(targetFile);
        	return getServerRoot(request) +"/" + picHome +"/" + yearMonth + "/" + fileName;
        }
        return null;
//        return getServerRoot(request) + restUrl + yearMonth + "/" + fileName;
    ...
    private static String getServerRoot(HttpServletRequest request){
        ...
        log.info("serverRoot: "+serverRoot);
        return serverRoot;
    }
}
